...
export async function getUnreadNot(userId: string | any) {
  const response = await axios.get(`${PRO_URL}/notification/unread/${userId}`);

  return response?.data;
}

export async function getUserById(id: string | any) {
  const user = await axios.get(`${PRO_URL}/users/find-by-id/${id}`);
...
export async function getCurrentReq({
  ...
  const response = await axios.get(
    `${PRO_URL}/request/current/${rId}/one/${cId}`
  );
  return response?.data;
}
...
export async function getUserCollections(userId: string) {
  const photos = await axios.get(
    `${PRO_URL}/article/user/collections/${userId}`
  );
  return photos.data;
}
export async function getUserArticles(userId: string) {
  const articles = await axios.get(
    `${PRO_URL}/article/user/articles/${userId}`
  );
